...
A GNOME Shell extension for people who use extended displays but do not want
the mouse to drift onto another monitor by accident.
...
- GNOME Shell 46 (currently tested and supported)
- Wayland
...
```bash
git clone https://github.com/incirci/monitor-pointer-lock
cd monitor-pointer-lock
make install
```
...
Monitor Pointer Lock is intended to support multiple modern GNOME Shell
versions. At present, GNOME Shell 46 on Ubuntu 24.04 is the only tested and
supported version. The extension does not claim compatibility with other GNOME
Shell versions until it has been validated on them.
